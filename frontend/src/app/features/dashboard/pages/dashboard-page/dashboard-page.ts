...
import { InventoryItem, ActivityEvent, MonthlyMetric, Order, PeriodMeta, Poster } from '../../../../models/erp.models';
...
export class DashboardPage implements OnInit {
  ...
  readonly activityTimeline = signal<ActivityEvent[]>([]);
  readonly periodLabel = computed(() => this.period.label());
  readonly salesColumnLabel = computed(() => this.period.salesColumnLabel());

  readonly kpiTrendRevenue = computed(() =>
    this.revenue() > 0 ? `${this.periodLabel()} · revenue` : '',
  );
  readonly kpiTrendOrders = computed(() =>
    this.orderCount() > 0 ? `${this.periodLabel()} · orders` : '',
  );

  readonly urgentPrintJobs = computed(() =>
    this.printJobs().filter(
      (job) =>
        job.stage !== 'Delivered' &&
        job.stage !== 'Cancelled' &&
        (job.priority === 'High' || job.priority === 'Rush'),
    ).length,
  );

  readonly kpiDeltaPendingJobs = computed(() => {
    if (this.pendingJobs() === 0) return '';
    const urgent = this.urgentPrintJobs();
    return urgent > 0 ? `${urgent} urgent` : '';
  });

  readonly kpiTrendInventory = computed(() =>
    this.inventoryAlerts() > 0 ? 'Purchase action needed' : '',
  );
  ...
  private loadDashboard(): void {
    ...
    this.api.get<DashboardApiResponse>(`/dashboard?${this.period.queryString()}`).subscribe({
      next: (payload) => {
        ...
        this.activityTimeline.set(payload.activityTimeline ?? []);
