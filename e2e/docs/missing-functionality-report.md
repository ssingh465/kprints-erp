# KPrints ERP — Missing Functionality Report

> **Discovery audit.** Initial catalogue of integration gaps, broken contracts, and quality risks found while building [`dependency-map.md`](dependency-map.md) and [`workflow-map.md`](workflow-map.md). This is **not** a complete production-readiness audit (that lands later); it is the working backlog that downstream test and fix work will close or escalate (document + recommend).
>
...
Total: **27 documented findings** (this list will grow as the workflow suites exercise the system).
...
- **Suggested fix (workflow critical-fix candidate):**
...
- **Test ownership:** workflow E2E tests must today assert _absence_ of any movement after order creation, then flip to asserting decrement once fix lands.
...
- **Test ownership:** API contract test should fire 10 parallel `POST /api/orders` and assert all succeed with distinct `orderNo`.
...
- **Test ownership:** workflow E2E tests should explicitly assert counter consistency after `DELETE /api/orders/:id` and after a `total` change.
...
- **Test ownership:** RBAC suite must run with demo mode disabled and confirm that all 8 personas hit their expected 403s.
...
- **Test ownership:** the readiness audit will record this as "schema-only feature". If implemented as part of workflow or UX work, add E2E.
...
- **Test ownership:** customer/orders workflow tests should currently assert coupon UI is "metadata-only" and skip discount math; flip on fix.
...
- **Test ownership:** DB-validation tests will today assert _absence_ of audit rows for these mutations; flip on fix.
...
- **Test ownership:** workflow test asserts the drift today (expected current behaviour), flips on fix.
...
- **Decision needed (readiness audit):** rename the module to "Purchase Ledger" in the UI to match reality, **or** scope a future `purchase_orders` table with proper PO → inventory_movement workflow.
...
- **Suggested fix (UX feedback / cross-cutting):** add a thin `ToastService` wrapper at `core/services/` and wire it into orders, customers, inventory, catalog, production (stage + operator), shipments, expenses, posters, coupons, vendors create/update/delete/upload success + error paths.
...
- **Where:** No middleware reviewed during the discovery audit indicates rate limits. The setup endpoints are protected by RBAC (only SUPER_ADMIN), but a panicked operator can still call them.
...
### G21. `lib/prisma` connection pool not surveyed during the discovery audit
...
- **Follow-up (API contract + performance):** read `backend/src/lib/prisma.ts` and confirm singleton + warm-up + `connection_limit`.
...
- **Suggested fix:** adopt `@hono/zod-openapi` so route schemas become the API contract; API contract tests can also consume the contract.
...
- **Where:** see G2. Magic numbers are also brittle for tests — every workflow test that constructs a fixture order number must be aware of the offset.
...
## 6. Cross-cutting observations (will inform the readiness audit)
...
| Storage objects orphaned on order delete | Upload tests must cover orphan-cleanup or document it |
| Coupon / Invoice features are **schema-only** | Readiness audit must either green-light implementation or schema cleanup |

---

## 7. Backlog snapshot (work-area routing)

Owner-area legend (used in the table below to route each gap without baking sequencing into the doc):

- **Workflow E2E** — cross-module Playwright suites that drive a real business flow end-to-end.
- **API contract** — backend HTTP contract tests (Vitest or Playwright `request`).
- **RBAC** — per-role access tests on UI + API.
- **DB validation** — direct Prisma assertions that mutations land (or don't) with correct shape and audit rows.
- **Uploads** — Supabase Storage + `ArtworkUpload` flows.
- **UX feedback** — toasts, error messages, and other user-facing feedback wiring.
- **Performance** — page/API timing thresholds.
- **Error handling** — simulated network / 4xx / 5xx degradation tests.
- **Test infra** — TEST DB reset, seed determinism, fixture plumbing.
- **Readiness audit** — final production-readiness report decision.
