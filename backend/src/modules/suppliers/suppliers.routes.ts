...
const suppliersApp = new Hono<{ Variables: AuthVariables }>();
const service = new SuppliersService();

suppliersApp.use('*', ...protect('vendors'));
...
suppliersApp.post('/', requireWrite('vendors'), zValidator('json', supplierSchema), async (c) => {
...
suppliersApp.put('/:id', requireWrite('vendors'), zValidator('json', supplierSchema.partial()), async (c) => {
...
suppliersApp.delete('/:id', requireWrite('vendors'), async (c) => {
  const id = c.req.param('id');
