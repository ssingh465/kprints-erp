...
const partnersApp = new Hono<{ Variables: AuthVariables }>();
const service = new PartnersService();

partnersApp.use('*', ...protect('finance'));
...
partnersApp.post('/', requireWrite('finance'), zValidator('json', partnerSchema), async (c) => {
...
partnersApp.put('/:id', requireWrite('finance'), zValidator('json', partnerUpdateSchema), async (c) => {
  const body = c.req.valid('json');
  try {
...
partnersApp.delete('/:id', requireWrite('finance'), async (c) => {
  try {
...
partnersApp.post('/:id/investments', requireWrite('finance'), zValidator('json', investmentSchema), async (c) => {
  const body = c.req.valid('json');
  try {
...
partnersApp.delete('/:id/investments/:investmentId', requireWrite('finance'), async (c) => {
  try {
