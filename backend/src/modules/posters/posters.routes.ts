...
const postersApp = new Hono<{ Variables: AuthVariables }>();
const service = new PostersService();

postersApp.use('*', ...protect('catalog'));
...
// Create Poster
postersApp.post('/', requireWrite('catalog'), zValidator('json', posterCreateSchema), async (c) => {
...
// Update Poster
postersApp.put('/:id', requireWrite('catalog'), zValidator('json', posterUpdateSchema), async (c) => {
  const id = c.req.param('id');
  const body = c.req.valid('json');
...
// Delete Poster
postersApp.delete('/:id', requireWrite('catalog'), async (c) => {
  const id = c.req.param('id');
