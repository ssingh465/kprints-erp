...
| `FRONTEND_URL` | Frontend base URL (invite/reset redirects) |
| `CORS_ORIGIN` | Allowed CORS origin(s); comma-separated in production |
...
| Component | Deploy from | Notes |
| --- | --- | --- |
| Backend API | `backend/` | Hono serverless via `backend/vercel.json` |
| Frontend | `frontend/` | Angular static build; set `apiBaseUrl: '/api'` in `environment.ts` |
| Database | Supabase | Use the **connection pooler** URL for `DATABASE_URL` on Vercel |

### Backend environment (Vercel project)

Set these in the Vercel backend project settings:
...
Ensure the build runs `prisma generate` (see `backend/package.json` build script).
...
If frontend and backend are separate Vercel projects, add a rewrite in the frontend project so `/api/*` proxies to the backend deployment URL.
...
Add the production callback to your Google Cloud OAuth client:

- `https://<project>.supabase.co/auth/v1/callback`

### RLS policies (manual step)

After schema migration, run `backend/prisma/supabase-rls.sql` in the Supabase SQL editor. This adds a lightweight RLS safety net on auth-related tables (`app_users`, `user_invitations`, `audit_logs`). ERP business tables remain protected by Hono middleware only.

### Super Admin bootstrap

After the first Supabase signup in production:
...
1. `ApiClientService` performs GET/POST/PUT/PATCH/DELETE against `environment.apiBaseUrl`.
2. `ApiRepository` implements the `Repository<T>` contract used by feature screens.
3. Use `/welcome` to enter demo mode or sign in; demo data seeds via `/api/setup/demo`.
4. Supabase Auth handles identity; Hono middleware enforces RBAC per role matrix.

## Current Scope

**Implemented:** Full ERP UI, REST API, Prisma schema, Supabase PostgreSQL persistence, Supabase Auth + RBAC, user invitations & approval workflow, demo/fresh setup, artwork uploads to Supabase Storage, dashboard and domain CRUD endpoints, audit logging, and production deployment docs.

**Not yet implemented:** GST, ledger accounting, journal entries, and TDS.
